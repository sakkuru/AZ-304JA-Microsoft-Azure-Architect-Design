...
1. Azure portal で、 **ロジック アプリ**を検索して選択し、「**ロジック アプリ**」 ブレードで、「**+ 追加**」 を選択します。
...
1. 「**コネクタとトリガーの検索**」テキスト ボックスを使用して **Azure Event Grid** を検索し、結果の一覧の 「**トリガー**」 列で、「**リソース イベントが発生したとき**」 Azure Event Gridトリガーを選択して、デザイナー ワークスペースに追加します。
...
1. **リソースイベントが発生したとき** タイルで、**新しいパラメーターを追加(Add new parameter)** を選択し、**サブスクリプション名** を選択します。
...
1. Azure portal に、新しくプロビジョニングされた Azure ロジック アプリのロジック アプリ デザイナー ブレードに **+ 新しいステップ** を選択します。 

1. アクション タイルの選択で、 **コネクタとトリガーを検索** テキストボックスを使用し、**条件**を検索し、結果のリストの**アクション**列で**条件**を選択してデザイナー ワークスペースに追加します。

1. 「**条件**」 タイルの右上隅にある省略記号を選択し、ポップアップ メニューで、「**名前の変更**」 を選択し、**リソース グループの仮想マシンが変更された場合は** に変更します。 

1. 条件の左側の 「**値の選択**」 テキスト ボックスを選択し、動的なコンテンツから **イベントの種類** を選択して入力します。

1. **「次の値に等しい」** が条件の中央要素に表示されていることを確認し、右側の 「**値の選択**」 テキスト ボックスに、監視する操作を表す値を入力します。
...
1. 「**Outlook.com**」 ウィンドウで、「**メールの送信 (V2)**」 タイルの右上隅にある省略記号を選択し、ポップアップ メニューで、「**名前の変更**」を選択し、「**メールの送信 (V2)**」 を 「**メールを送信**」 というテキストに置き換えます。 
...
1. 「**メールを送信**」 ウィンドウに、次の設定を指定し、「**保存**」 を選択します。
    ...
    | 本文 | (以下のように入力します)  **リソース グループ:** と入力し、**「メールを送信」** ペインの右側の **「動的コンテンツ」** 列の下にある検索テキストボックスで、**トピック** を入力して選択  **イベント タイプ:** と入力し、**「メールを送信」** ペインの右側の **「動的コンテンツ」** 列の下にある検索テキスト ボックスで、**イベントの種類** を入力して選択  **イベントID:** と入力し、**「メールの送信」** の右側にある **「動的コンテンツ」** 列の下にある検索テキスト ボックスで、**ID** を入力して選択  **イベント時間:** と入力し、**「メールを送信」** の右側にある **「動的コンテンツ」** 列の下にある検索テキスト ボックスで、**イベント時間** を入力して選択|
...
1. **リソース_イベントが発生したとき** ブレードで、 **コールバック URL [POST]** テキスト ボックスの値をコピーします。

1. Azure portal に **az30304a-LabRG** リソース グループに移動し、メニューから **イベント**をクリックします。
