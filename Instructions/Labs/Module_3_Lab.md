...
- ツールの範囲: 評価と移行のためのさまざまなツール。ツールには Azure Migrate が含まれます。
- サーバー評価と Azure Migrate: サーバー移行。Azure Migrate は、他の Azure サービスや、他のツールやパッケージソフトウェア開発企業 (ISV) オファリングと統合されます。
...
1. 「**新しい仮想マシンウィザード**」 の 「**概要**」 ページで、「**終了**」 を選択します。
...
- SMB 共有を使用して VHD を保存していますか？(Do you use SMB share(s) to store the VHDs?)

- Azure Migrate と Hyper-V ホスト通信用の非管理者ローカルユーザーを作成しますか?(Do you want to create non-administrator local user for Azure Migrate and Hyper-V Host communication?) 
...
1. 「**マシンの検出**」ブレードの「**アプライアンスに名前を指定する**」テキスト ボックスで「**az30308avma1任意の文字列**」(この名前は Key コンテナの名前にも使用されるため、az30308a-vma1-ja など、後ろにイニシャル等を追記して一意の名称にしてください）と入力し、「**キーの生成**」を選択します。
...
1. ダウンロードが完了したら、ダウンロードした .ZIP ファイルのコンテンツを **F:\VMs** フォルダーに解凍します。 
...
1. **仮想マシンのインポート** ウィザードの**概要**ページで、「**終了**」を選択します。
...
1. **az30308a-hv-vm** へのリモート デスクトップ セッション内で、Microsoft Edge ウィンドウを起動し、[https://`IPaddress`:44368](https://`IPaddress`:44368) に移動します。ここで、`IPadress`プレースホルダーは、前のステップで識別した IP アドレスを表します。
...
1. **az30308a-hv-vm** へのリモート デスクトップ セッション内で、 Azure portal を表示している Internet Explorer ウィンドウで、**Azure Migrate** に戻ります。**|**「**サーバー**」ブレードで「**更新**」を選択し、「**Azure Migrate: サーバーの評価**」タイルで「**評価(Asses)**」を選択します。

   >**注**: もう一度ページを更新してください。 

1. 「**サーバーの評価**」ブレードで、「**評価プロパティ**」ラベルの隣にある「**編集**」リンクを選択します。
...
1. 「**新規作成**」オプションが選択されていることを確認し、グループ名を「**az30308a-assessment-group**」に設定します。

1. グループに追加するマシンのリストで「**az30308a-vm1**」を選択します。

1. 「**次へ**」をクリックしてから、「**評価の作成**」をクリックします。 

1. 「**Azure Migrate | サーバー**」ブレードに戻り、「**更新**」を選択して画面をリフレッシュします。「**Azure Migrate: サーバーの評価**」タイルで、「**評価**」行に「**1**」のエントリが含まれていることを確認してから、それを選択します。
...
1. **az30308a-hv-vm** へのリモート デスクトップ セッション内で、 Azure portal を表示している Internet Explorer ウィンドウで、「**Azure Migrate | サーバー**」 ブレードに戻り、「**移行ツール**」 セクションの下にある 「**ここをクリックしてください**」 のリンクを選択します。 
...
1. 「**マシンの検出**」ブレードの「**1. Hyper-V ホスト サーバーを準備する**」ステップで、文章の最初の**ダウンロード** リンク（ダウンロードボタンではありません）を選択し、Hyper-V レプリケーション プロバイダー ソフトウェア インストーラーをダウンロードします。
...
1. レプリケーションのステータスを監視するには、「**Azure Migrate | サーバー**」ブレードに戻り、「**Azure Migrate: サーバーの移行**」タイルで「**サーバーをレプリケートしています**」エントリを選択します。

1.「**Azure Migrate: Server Migration | マシンのレプリケート**」で、レプリケート中のマシンのリストの「**状態**」列を調べます。

1. 初期レプリケートが終了し、ステータスが 「**プロテクト**」 に変わるまで待ちます。この操作は 15 分間程度かかる場合があります。
...
1. **az30308a-vm1** マシンのレプリケート ブレードで、「**テスト移行**」を選択します。

1. 「**移行テスト**」ブレードの「**仮想ネットワーク**」ドロップダウン リストで「**az30308c-test-vnet**」を選択し、「**テスト移行**」をクリックします。
...
1. Azure portal で、「**Azure Migrate: サーバーの移行 | マシンのレプリケート**」に戻り、「**更新**」を選択し、**az30308a-vm1** 仮想マシンが「**テストフェールオーバーのクリーンアップが保留中**」状態でリストされていることを確認します。

1. 「**Azure Migrate: Server Migration | マシンのレプリケート**」ブレードで、**az30308a-vm1** 仮想マシンを表すエントリを選択します。

1. **az30308a-vm1** マシンのレプリケート」ブレードで、「**テスト移行をクリーンアップ**」を選択します。

1. 「**テスト移行のクリーンアップ**」ブレードで、「**テストが完了しました**」チェックボックスを選択します。次に、「**テストのクリーンアップ**」を選択します。

1. テスト フェールオーバー クリーンアップ ジョブが完了したら、**az30307a-vm1** マシンのレプリケート ブレードを表示するページを更新して、ツールバーの**移行**アイコンが利用可能になったことを確認します。

1. **az30307a-vm1** マシンのレプリケート ブレードで、「**移行**」リンクを選択します。 

1. 「**移行**」ブレードの、「**仮想マシンをシャットダウンし、データの損失のない計画された移行を実行しますか?**」ドロップダウン リストで、「**はい**」を選択してから、「**移行**」を選択します。

1. 移行のステータスを監視するには、「**Azure Migrate | サーバー**」 ブレードに戻り、「**Azure Migrate: Server Migration**」タイルで、「**サーバーをレプリケートしています**」エントリを選択します。

1.「**Azure Migrate: Server Migration | マシンのレプリケート**」 で、レプリケート中のマシンのリストの 「**状態**」 列を調べます。ステータスが「**予定されていたフェールオーバーが終了しました**」になっていることを確認します。
